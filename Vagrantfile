...
if File.exist?('./vagrant.local.yml') then
  settings = YAML.load_file './vagrant.local.yml'
end
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.box = 'pingworks_ubuntu-14.04_chef-11.16.4-1'

  # The url from where the 'config.vm.box' box will be fetched if it
  # is not a Vagrant Cloud box and if it doesn't already exist on the
  # user's system.
  config.vm.box_url = "http://vagrant.pingworks.net/pingworks_ubuntu-14.04_chef-11.16.4-1.box"
